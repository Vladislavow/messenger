<template>
    ...
    >
        <div class="content">
            ...
            <span class="text" v-if="contentArr.length>0" >
                <span  v-for="(item, index) in contentArr" :key="index">
                {{ !item.is_link ? item.content :''}}
                  <a v-if="item.is_link" target="_blank" :href="item.content">{{item.content}}</a> 
                </span>
            </span>
            <span v-if="contentArr.length==0" class="text">
                {{message.content}}
            </span>
            <div class="statuses">
                <span
    ...
    methods: {
        handleLinks() {
            ...
            if (this.message.content) {
                // this.contentArr = [];
                // this.contentArr = this.message.content.match(expression);
                ...
                if (links && links.length > 0) {
                    links.forEach((link) => {
                        if (this.contentArr.length == 0) {
                            ...
                            let temp =
                                this.contentArr[
                                    this.contentArr.length - 1
                                ].content.split(link);
                            this.contentArr.splice(
                                this.contentArr.length - 1,
                                1
                            );
                            this.contentArr.push({
